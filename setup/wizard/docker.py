...
def step_ports(args, answers):
    ...
    options = [
        ("Development",
         "Safe local ports: HTTP 8080, HTTPS 8443, SMTP 2525, DNS 5354. No root required.",
         "dev"),
        ("Production",
         "Standard ports: HTTP 80, HTTPS 443, SMTP 25, DNS 53. Requires root.",
         "prod"),
    ]
    return select_prompt(
        "Which port profile should be used?",
        "Use development ports locally. Switch to production on a real server.",
        options, current or "dev", current is not None,
    )
...
def write_env(path, answers):
    """Write a Docker .env file from wizard answers (skip wizard-only _ keys)."""
    env_vars = {k: v for k, v in answers.items() if not k.startswith("_")}
...
def run(env_path):
    ...
    print(f"  {gray_desc('Re-run this wizard at any time: python3 setup/wizard docker')}\n")
