...
apt_install_cached "ssl" openssl
...
if [ ! -f "$STORAGE_ROOT/ssl/ssl_certificate.pem" ]; then
	# Build the Subject Alternative Name list. PRIMARY_HOSTNAME is always included.
	# SSL_EXTRA_SANS can be set (comma-separated) to add extra names - used in
	# Docker so webmail can verify Dovecot's cert when connecting via IMAPS to
	# the "mail" container service name.
	SAN="DNS:$PRIMARY_HOSTNAME"
	if [ -n "${SSL_EXTRA_SANS:-}" ]; then
		for _san in ${SSL_EXTRA_SANS//,/ }; do
			if [[ "$_san" =~ ^[0-9] ]]; then
				SAN="${SAN},IP:${_san}"
			else
				SAN="${SAN},DNS:${_san}"
	...
	# Generate the self-signed certificate with the SAN extension.
