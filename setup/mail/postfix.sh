...
# * `postgrey`: Only installed for SPAM_FILTER=spamassassin. Rspamd handles
#   greylisting internally via Redis.
# * `ca-certificates`: A trust store used to squelch postfix warnings about
#   untrusted opportunistically-encrypted connections.
echo "Installing Postfix (SMTP server)..."
if [ "${SPAM_FILTER:-rspamd}" = "spamassassin" ]; then
    apt_install_cached "postfix" postfix postfix-sqlite postfix-pcre postgrey ca-certificates
else
    apt_install_cached "postfix" postfix postfix-sqlite postfix-pcre ca-certificates
fi
...
#
# The submission milter depends on SPAM_FILTER: Rspamd on 11332, OpenDKIM on 8891.
# OpenDMARC is deliberately excluded from submission (outbound) - it only applies
# to incoming mail.
if [ "${SPAM_FILTER:-rspamd}" = "rspamd" ]; then
    _SUBMISSION_MILTER="inet:127.0.0.1:11332"
else
    _SUBMISSION_MILTER="inet:127.0.0.1:8891"
fi
setup/tools/editconf.py /etc/postfix/master.cf -s -w \
	"smtps=inet n       -       -       -       -       smtpd
	  -o smtpd_tls_wrappermode=yes
	  -o smtpd_sasl_auth_enable=yes
	  -o syslog_name=postfix/submission
	  -o smtpd_milters=$_SUBMISSION_MILTER
...
cp setup/conf/mail/postfix_outgoing_mail_header_filters /etc/postfix/outgoing_mail_header_filters
...
# ### Postscreen
#
# Postscreen sits in front of port 25 and rejects obviously bad senders
# before they can waste resources on a full SMTP conversation. It performs:
# * DNSBL checks (Spamhaus zen, Barracuda)
# * SMTP greeting delay (catches zombies that talk before the banner)
# No new dependencies - it is built into Postfix.
...
# For the SpamAssassin path, mail goes to spampd (port 10025) which runs
# SpamAssassin and then relays to Dovecot. For Rspamd, Rspamd acts as a
# milter so mail goes directly to Dovecot LMTP - no relay hop needed.
if [ "${SPAM_FILTER:-rspamd}" = "rspamd" ]; then
    setup/tools/editconf.py /etc/postfix/main.cf \
...
setup/tools/editconf.py /etc/postfix/main.cf -e lmtp_destination_recipient_limit=
...
# Recipient restrictions differ by spam filter:
# * SpamAssassin path includes Postgrey (10023) for greylisting.
# * Rspamd path drops Postgrey - Rspamd handles greylisting via Redis.
# Both paths keep the Dovecot quota policy service (12340).
if [ "${SPAM_FILTER:-rspamd}" = "spamassassin" ]; then
    setup/tools/editconf.py /etc/postfix/main.cf \
...
# Postgrey is only used when SPAM_FILTER=spamassassin. Rspamd handles
# greylisting internally via Redis.
if [ "${SPAM_FILTER:-rspamd}" = "spamassassin" ]; then
	...
	setup/tools/editconf.py /etc/default/postgrey \
	...
	if [ ! -d "$STORAGE_ROOT/mail/postgrey/db" ]; then
		# Stop the service (|| true: postgrey may not be running on a fresh container)
	...
	# Ensure permissions are set on first creation only - postgrey db grows with traffic.
	if [ ! -d "$STORAGE_ROOT/mail/postgrey/db" ] || [ "$(stat -c %U "$STORAGE_ROOT/mail/postgrey")" != "postgrey" ]; then
...
fi # end SPAM_FILTER=spamassassin
