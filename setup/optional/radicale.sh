...
_radicale_pip_hash=$(hash_files "$PWD/setup/optional/radicale.sh")
if needs_build "radicale-pip" "$_radicale_pip_hash"; then
	...
	mark_built "radicale-pip" "$_radicale_pip_hash"
fi
...
# Auth plugin: validates credentials via the management daemon's /auth/verify
# endpoint. The management host defaults to 127.0.0.1 (bare metal) but can be
# overridden via 'management_host' in the [auth] section of the Radicale config.
...
The management host defaults to 127.0.0.1 (bare metal loopback) but can be
overridden via the 'management_host' key in the [auth] section of the Radicale
config. In Docker the radicale entrypoint sets management_host = management so
that authentication reaches the management container by service name.
"""
import logging
import urllib.error
import urllib.parse
...
# Per-user write locks so concurrent DAV clients don't interleave writes.
...
class _OxiCollection(storage.BaseCollection):
    def __init__(self, path: str, data_dir: str, email: str, coll_type: str):
        ...
        # Open one connection per collection instance; closed on GC.
...
# Storage: when oxi is the webmail client the custom plugin bridges its per-user
# SQLite databases so contacts/calendar show up in both the web UI and DAV clients.
# For all other clients use Radicale's standard multifilesystem storage so contacts
# saved via DAV clients (DAVx5, iOS, Thunderbird) are persisted independently.
mkdir -p /etc/radicale
if [ "${WEBMAIL_CLIENT:-oxi}" = "oxi" ]; then
    _RADICALE_STORAGE_BLOCK="type = radicale_miab.storage"
else
    # Store collections at /var/lib/radicale so the path is outside /home and
    # compatible with ProtectHome=true in the systemd sandbox. Backups are handled
    # by the backup script which explicitly includes this path.
    mkdir -p /var/lib/radicale/collections
    chown www-data:www-data /var/lib/radicale/collections
    _RADICALE_STORAGE_BLOCK="type = multifilesystem
...
# Override to the 'management' service name in Docker via the container entrypoint.
...
# Systemd unit.
STORAGE_ROOT="$STORAGE_ROOT" RADICALE_VENV="$RADICALE_VENV" envsubst '${RADICALE_VENV} ${STORAGE_ROOT}' < setup/conf/systemd/radicale.service > /lib/systemd/system/radicale.service

# Some VPS kernels (OpenVZ, LXC) don't support mount namespaces, which causes
# PrivateTmp=true and ProtectSystem=strict to fail with 226/NAMESPACE.
# Detect this at install time and write a drop-in that disables only those directives.
mkdir -p /etc/systemd/system/radicale.service.d
DROPIN=/etc/systemd/system/radicale.service.d/no-namespace.conf
if unshare -m true 2>/dev/null; then
    # Namespaces supported - remove any previously applied drop-in
    rm -f "$DROPIN"
else
    echo "  Note: kernel lacks mount namespace support - applying reduced sandbox configuration"
    cat > "$DROPIN" << 'EOF'
[Service]
PrivateTmp=false
ProtectSystem=false
