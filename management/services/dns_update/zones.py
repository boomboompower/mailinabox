...
def build_zone(domain, domain_properties, additional_records, env, is_zone=True):
	...
	if domain_properties[domain]["auto"]:
		...
		if domain.startswith("autodiscover."): a_expl = "Provides email client autodiscovery support."
	defaults = [
	...
	if domain_properties[domain]["mail"]:
		...
		if not has_rec(None, "TXT", prefix="v=spf1 "):
			...
			if _spf_include:
				spf_value = f"v=spf1 mx include:{_spf_include} -all"
				spf_note = f"Recommended. Specifies that the box and {_spf_include} are permitted to send @{domain} mail."
			else:
				spf_value = "v=spf1 mx -all"
				spf_note = f"Recommended. Specifies that only the box is permitted to send @{domain} mail."
			records.append((None, "TXT", spf_value, spf_note))
