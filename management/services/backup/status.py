...
def _restic_backup_status(env, config):
	...
	code, snapshots_json = shell('check_output', [
		RESTIC, "-r", repo, "snapshots", "--json", *extra_args,
		], restic_env, trap=True, capture_stderr=True)
	if code != 0:
	...
	unmatched_file_size = 0
	code, stats_json = shell('check_output', [
		RESTIC, "-r", repo, "stats", "--mode", "raw-data", "--json", *extra_args,
		], restic_env, trap=True)
	...
	if target.scheme == "rsync":
		...
		if 'Permission denied (publickey).' in listing:
			...
			reason = ("Unknown error."
					"Please check running 'management/services/backup --verify'"
