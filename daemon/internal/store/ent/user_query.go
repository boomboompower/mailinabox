...
type UserQuery struct {
	...
	withMailKeySlots        *MailKeySlotQuery
	withEncryptionSetups    *EncryptionSetupQuery
...
func (_q *UserQuery) loadTotpCredentials(ctx context.Context, query *TOTPCredentialQuery, nodes []*User, init func(*User), assign func(*User, *TOTPCredential)) error {
	...
	query.Where(predicate.TOTPCredential(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TotpCredentialsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_totp_credentials
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_totp_credentials" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_totp_credentials" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadWebauthnCredentials(ctx context.Context, query *WebAuthnCredentialQuery, nodes []*User, init func(*User), assign func(*User, *WebAuthnCredential)) error {
	...
	for _, n := range neighbors {
		fk := n.user_webauthn_credentials
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_webauthn_credentials" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_webauthn_credentials" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadWebauthnChallenges(ctx context.Context, query *WebAuthnChallengeQuery, nodes []*User, init func(*User), assign func(*User, *WebAuthnChallenge)) error {
	...
	query.Where(predicate.WebAuthnChallenge(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.WebauthnChallengesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_webauthn_challenges" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadMailKeySlots(ctx context.Context, query *MailKeySlotQuery, nodes []*User, init func(*User), assign func(*User, *MailKeySlot)) error {
	...
	query.withFKs = true
	query.Where(predicate.MailKeySlot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.MailKeySlotsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_mail_key_slots
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_mail_key_slots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_mail_key_slots" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *UserQuery) loadEncryptionSetups(ctx context.Context, query *EncryptionSetupQuery, nodes []*User, init func(*User), assign func(*User, *EncryptionSetup)) error {
	...
	query.withFKs = true
	query.Where(predicate.EncryptionSetup(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.EncryptionSetupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_encryption_setups
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_encryption_setups" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_encryption_setups" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
