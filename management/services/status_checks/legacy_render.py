...
_ACRONYM_FIX = {
    "Dns": "DNS", "Smtp": "SMTP", "Mx": "MX", "Tls": "TLS",
    "Ssh": "SSH", "Imap": "IMAP", "Miab": "MIAB", "Rbl": "RBL",
    "Ipv4": "IPv4", "Ipv6": "IPv6", "Lda": "LDA",
    "Http": "HTTP", "Https": "HTTPS", "Ufw": "UFW",
}
    ...
    for category in sorted(by_category):
        ...
        for r in sorted(by_category[category], key=lambda r: (r.name, r.domain or "")):
            ...
            if item_type in {"error", "warning"}:
                ...
                extra = [
                    {"text": f"{s.name}: {s.message}" if s.message else s.name, "monospace": False}
                    for s in failing[1:]
                ]
            else:
                ...
                extra = [
                    {"text": s.name, "monospace": False}
                    for s in passing[1:]
                    if s.name.lower() != label.lower()
                ]
