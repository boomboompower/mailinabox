...
def scan_mail_log(env):
	...
	try:
		...
		collector["known_addresses"] = set(mailconfig.get_mail_users(env)) | {alias[0] for alias in mailconfig.get_mail_aliases(env)}
	except ImportError:
		pass
	...
	print("{scan_count} Log lines scanned, {parse_count} lines parsed in {scan_time:.2f} seconds\n".format(**collector))
	...
	if collector["logins"]:
		...
		print_user_table(
			...
			data=[
				(
					protocol_name,
					[
						round(u["totals_by_protocol"][protocol_name] / (u["latest"] - u["earliest"]).total_seconds() * 60 * 60, 1) if (u["latest"] - u["earliest"]).total_seconds() > 0 else 0  # prevent division by zero
						for u in data.values()
					],
				)
				for protocol_name in all_protocols
			],
			sub_data=[("Protocol and Source", [[f"{protocol_name} {host}: {count} times" for (protocol_name, host), count in sorted(u["totals_by_protocol_and_host"].items(), key=lambda kv: -kv[1])] for u in data.values()])],
			activity=[(protocol_name, [u["activity-by-hour"][protocol_name] for u in data.values()]) for protocol_name in all_protocols],
			earliest=[u["earliest"] for u in data.values()],
			latest=[u["latest"] for u in data.values()],
		...
		print_time_table(all_protocols, [accum[protocol_name] for protocol_name in all_protocols])

	if collector["postgrey"]:
		msg = "Greylisted Email {:%Y-%m-%d %H:%M:%S} and {:%Y-%m-%d %H:%M:%S}"
		print_header(msg.format(state.START_DATE, state.END_DATE))

		print(textwrap.fill("The following mail was greylisted, meaning the emails were temporarily rejected. Legitimate senders must try again after three minutes.", width=80, initial_indent=" ", subsequent_indent=" "), end='\n\n')
		...
		print_user_table(
			users,
			data=[("received", received), ("sender", senders), ("delivered", [str(d) or "no retry yet" for d in delivered_dates]), ("sending host", sender_clients)],
			delimit=True,
		)
